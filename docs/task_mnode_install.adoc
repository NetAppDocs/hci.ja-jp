...
... 追加するデフォルトタイムサーバーごとに新しい行を追加してください。デフォルトタイムサーバーは、link:task_mnode_install.html#step-4-set-up-the-management-node["後の手順"]で使用するストレージクラスターで使用されるNTPサーバーと同じである必要があります。
+
[listing]
----
vi /etc/ntp.conf
...
NOTE: 時刻同期設定が完了した後は、NTP を編集しないでください。管理ノードで link:task_mnode_install.html#step-4-set-up-the-management-node["Setup コマンド"] を実行すると、NTP に影響するためです。
...
*** *--storage_mvip (-sm) [MVIPアドレス]*：Elementソフトウェアを実行しているストレージクラスタの管理仮想IPアドレス（MVIP）。管理ノードを、link:task_mnode_install.html#step-3-configure-time-sync["NTP サーバの設定"]で使用したストレージクラスタと同じ構成にします。
