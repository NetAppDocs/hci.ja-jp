...
NOTE: 時刻同期設定が完了した後は、NTP を編集しないでください。管理ノードで <<step-6,再導入コマンド>> を実行すると、NTP に影響するためです。
...
. [[step-6]]クラスター上でホストされている永続ボリュームに接続し、以前の管理ノード構成データを使用してサービスを開始するには、管理ノード再デプロイコマンドを設定して実行します：
+
