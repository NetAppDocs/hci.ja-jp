...
既存の Witness Node を Element 12.8 以降にインプレース アップグレードする場合は、アップグレードする前に、環境に応じて Witness Node 上の VM ハードウェア バージョンが ESXi 6.7 (VM ハードウェア バージョン 14) 以降と互換性があることを確認する必要があります。見るlink:task_hcc_upgrade_management_node.html#upgrade-vm-hardware["VMハードウェアバージョンのアップグレード"]。
